% ridgepack_JAMES_figure11 - Generates Figure 11 in JAMES Variational Ridging paper
% 
...
for i=1:length(hfii)
 ...
 if i==length(hfii)
  text(ssk(end),phk(end)-0.02,vrk(end)*1.3,...
            '$H_{K_{max}}=16\sqrt{h_{F_d}}\qquad$',...
            'Color',cols(5,:),...
            'Fontsize',10,...
            'Rotation',-18.5,...
 ...
 if i==length(hfii)
  text(sss(end),phs(end),vrs(end),'$H_{S_{max}}=5.24\sqrt{h_{F}}\qquad$',...
            'Color','w',...
            'Fontsize',10,...
            'Rotation',-18.5,...
