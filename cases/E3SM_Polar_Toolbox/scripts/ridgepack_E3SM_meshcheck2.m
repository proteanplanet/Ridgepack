...
for setting=plotchoice
 ...
 for gridchoice=grids
  ...
  if largescale
   ...
   % add isobaths
   if bathymetry
    hk=ridgepack_e3smsatthreshold(ncisobath50,centlat,centlon,horizon,[0 0.9 0]);
    hk=ridgepack_e3smsatthreshold(ncisobath30,centlat,centlon,horizon,[0.9290 0.6940 0.1250]);
   ...
   if bathymetry
    ...
    % add coast
    ridgepack_e3smsatcoast(nccoast,centlat,centlon,horizon)
    ridgepack_multilegend([h hb hg],{'E3SM-HR V1 coastline','30m','50m'},'South')
  ...
  %ridgepack_multialign(gcf,[num2str(setting),': ',zoom{setting}.name,', ',maintitle]);
