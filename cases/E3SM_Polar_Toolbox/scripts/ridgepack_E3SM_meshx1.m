...
for bk=1:length(bathymetrys)
 ...
 sector{7}.centlat=69; % degrees north
 sector{7}.centlon=-90; % degrees east
 sector{7}.horizon=45; % degrees of satellite horizon (0-90)
 sector{7}.altitude=1; % Mean Earth radius multiple
 sector{7}.annotation=3; % no annotation
 ...
 for setting=plotchoice
  ...
  for gridchoice=grids
   ...
   elseif strcmp(char(fileg{gridchoice}.name),'RRS6to18E3r6')
    ...
    shiplocs=[1 2 3 4 6];
   elseif strcmp(char(fileg{gridchoice}.name),'ARRM')
    gridloc=['/Users/afroberts/data/MODEL/E3SM/ARRM/grid'];
    gridfile='mpaso.ARRM10to60E2r1.rstFrom1monthG-chrys.220802.nc';
    shiplocs=[];
   else
   ...
   if bathymetry
    ...
    x=dir(mbatname);
    %minIso=ceil(100*min(ncvert.bottomDepth.data(:)))./100;
    minIso=ceil(min(ncvert.bottomDepth.data(:)));
...
%                  sum(ncvert.restingThickness.data(1:ncvert.maxLevelCell.data(iCell)-1, iCell));
   ...
   if largescale
    ...
    if bathymetry
     ...
     for j=4
      ...
      ridgepack_sathorizon(centlat,centlon,horizon,...
                           satlat,satlon,sathor,[0.8500 0.3250 0.0980],'');
     end
     ...
     if iceshelfplot
      hs=ridgepack_e3smsatthreshold(nciceshelf,centlat,centlon,horizon,...
                                    0.45*[1 0 0]);
      if gridl==1
       legend([hs ht hg hd],{'Ice Shelf Edge',mdepthc,'50m','500m isobaths'},...
            'Location','SouthOutside','Orientation','Horizontal')
       legend('boxoff')
      elseif gridx==gridl
       ridgepack_multilegend([hs ht hg hd],...
                  {'Ice Shelf Edge',mdepthc,'50m','500m isobaths'},'South')
      end
     else
      if gridl==1
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     ...
     elseif sector{setting}.annotation==3

      %for years=[1980 1990 2000 2010 2020 2023]
      for months=[3,9]
      for years=[1980 2023]

       cd(['/Volumes/CICESatArray/data/SATELLITE/processed/NSIDC_Ice_Edge/extent_N_',...
           num2str(years),'0',num2str(months),'_polyline_v3.0'])
       extent = shaperead(['extent_N_',num2str(years),'0',num2str(months),...
                           '_polyline_v3.0.shp'],'UseGeoCoords',false);

       cd('/Volumes/CICESatArray/data/SATELLITE/processed/NSIDC_Ice_Edge')
       exgrid = ridgepack_clone('NSIDC0771_LatLon_PS_N25km_v1.0.nc')

       clear xidxx yidxx xidxn yidxn lats lons latfinal lonfinal

       for i=1:length(extent.X)
        xidxx=ridgepack_nearestval(extent.X(i),exgrid.x.data);
        if ~isempty(xidxx) 
         xidxn(i,1:2)=xidxx;
        else
         xidxn(i,1:2)=[NaN NaN];
       ...
       for i=1:length(extent.Y)
        yidxx=ridgepack_nearestval(extent.Y(i),exgrid.y.data);
        if ~isempty(yidxx) 
         yidxn(i,1:2)=yidxx;
        else
         yidxn(i,1:2)=[NaN NaN];
       ...
       latfinal=[];
       lonfinal=[];
       for ix=1:length(extent.X)
        for jx=1:2
         if ~isnan(xidxn(ix,jx))
          lats(jx)=exgrid.latitude.data(yidxn(ix,jx),xidxn(ix,jx));
          lons(jx)=exgrid.longitude.data(yidxn(ix,jx),xidxn(ix,jx));
         else
          lats(jx)=NaN;
          lons(jx)=NaN;
         end
        end
        [latfinal(ix),lonfinal(ix)]=meanm(lats,lons);
       end

       [x,y,z,phi,theta]=...
          ridgepack_satfwd(latfinal,lonfinal,centlat,centlon,horizon,1.01*altitude);
       
       if months==03
        if years==1980
         hhs=plot3(x,y,z,'-','color',[0 0.4470 0.7410],'linewidth',1.5);
        elseif years==2023
         plot3(x,y,z,':','color',[0 0.4470 0.7410],'linewidth',1.5);
        end
       elseif months==09
        if years==1980
         hhm=plot3(x,y,z,'-','color',[0.8500 0.3250 0.0980],'linewidth',1.5);
        elseif years==2023
         plot3(x,y,z,':','color',[0.8500 0.3250 0.0980],'linewidth',1.5);
        end
       end
      ...
      legend([hhs,hhm],{'March','September Observed Sea Ice Extent'},...
               'Location','SouthOutside','Orientation','Horizontal')
      legend('boxoff')
