...
titletab='Evaluation of Icepack 1.5';

filetabe='icepack';

ensemblenames={'windstressold','windstressnew'};

legnames={'Anistropic','Isotropic'};

ensemblecases={[1],[2]};

yearrange={[0001 0089],[0001 0089]};
...
% case names
casenames={'20240528.v3.LR.piC-spinup-oaniso.chrysalis',...
           '20240528.v3.LR.piC-spinup-oisotr.chrysalis'}

dirnames={'/Users/afroberts/work/windstressold',...
          '/Users/afroberts/work/windstressnew'}
...
if length(yearrange)>3
 error('Year range must only include three windows')
end
...
for kcols=1:maxcols
 ...
 if kcols==1
  ...
  if yi>1
   ...
   for sdf=1:length(legnames)
    years=yearrange{sdf};
    legendnames{sdf}=[num2str(years(1)),'-',num2str(years(2)),' ',char(legnames{sdf})];
   end
  else
   years=yearrange{1};
  ...
  legend(h([1:length(legendnames)]),legendnames{1:length(legendnames)},'location','southwest','FontSize',8);
  legend('boxoff');
 end
...
if observations
 obsyeartag=['obs-',num2str(yearsto,'%4.4i'),'-',num2str(yeareno,'%4.4i'),'.'];
else
 obsyeartag=[];
end
