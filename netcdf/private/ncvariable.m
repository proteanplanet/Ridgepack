function ncvariable(ncid,nc,rec,recp,recl)

% NCVARIABLE - Adds name, type and attributes of new variable to netcdf file
%
% function ncvariable(ncid,nc,rec,recp)
%
...
% nc      - Data structure to be written to netcdf file (see ncstruct)
%
% Optional inputs:
...
% and these are provided in the documentation for ncstruct ('help ncstruct').
%
...
% ncstruct for a description of the netcdf data structure.
%
...
% to help identify time when using ncdump.
timestamp=1;
...
xvarnames=nccellcat(variablenames);
...
for m = 1:numbervariables
 ...
 if isempty(nc.(name).dimension);
   ...
   if strcmpi(name,'time') | strcmpi(name,'time_bounds')
    ...
    % Convert time to the chosen units in the ncstructure
    timeshape=size(nc.(name).data);
    [nc.(name).data,units,calen]=nctimeconvert(nc.(name).data(:),nc.(name).units,...
                                               nc.(name).calendar,1);
