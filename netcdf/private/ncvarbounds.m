function [dimremove,template,tempsize,endsize,ant,time]=ncvarbounds(nc,dimnames,bounds,name)

% NCVARBOUNDS - Finds reduction bounds for a specific variable in a netcdf structure
%
% function [dimremove,template,tempsize,endsize,ant,time]=ncvarbounds(nc,dimnames,bounds,name)
%
...
% form. 
%
% The main purpose of this function is to be called by the function ncreduce for
% reducing datasets. See ncreduce for further explanation.
%
% Input:
...
for j=1:length(dimremove)
	 ...
	 if strcmp('time',char(dimnames{j})); 
	  ...
	  units=ncunits(nc,char(dimnames{j})); % output units for plotting
