function [nc]=ncgroom(nc, name)

% NCGROOM - Grooms a netcdf structure for writing or after reading netcdf files
%
% function [nc]=ncgroom(nc, name)
% 
% This function grooms individual components of a netcdf structure (see 
% ncstruct for explanation of this structure and required inputs).  The
...
if ~isfield(nc.(name),'type') & isfield(nc.(name),'data')
  nc.(name).type=ncinvert(class(nc.(name).data));
end
