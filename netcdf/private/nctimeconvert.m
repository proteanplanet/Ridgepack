function [newtime,units,CFcalendar]=nctimeconvert(time,units,CFcalendar,tofrom)

% NCTIMECONVERT - Convert CF convention time to serial time
%
% function [newtime,units,CFcalendar]=nctimeconvert(time,units,CFcalendar,tofrom)
%
...
% with the ncgen/ncdump utilities that need to be installed on the computer being 
...
% 'gregorian','standard','no_leap','365_day' - where the minimum time record < 1582-10-15
% [These conversions are conducted externally in udunits via a call to nccftime]
%
...
% the ncdump and ncgen utilities are used by way of a call to a seperate function 
% nccftime.m as part of the icepack package.  These calls are made without need for mexnc
...
if any(strcmpi(CFcalendar,{'standard','gregorian','proleptic_gregorian','noleap','365_day'}))
	   ...
	   error('Specifying the conversion direction for time')
         ...
         elseif ~tofrom
          ...
          disp(['Processing ',CFcalendar,' calendar units ',...
               [compstr,datestr(basetime)],' UTC using ncdump/ncgen/udunit'])

	  newtime=nccftime(time,units,CFcalendar);
   ...
   if isempty(basetime)
     ...
     disp(['ERROR: Unable to find any of these time units: ',nccellcat(baseunits)])
     error(['Unable to find any of these time units: ',nccellcat(baseunits)])
...
elseif any(strcmpi(CFcalendar,{'all_leap','366_day','julian','360_day'}))
   ...
   elseif ~tofrom
    ...
    disp(['Processing ',CFcalendar,' calendar externally using ncgen/ncdump/udunits'])
    newtime=nccftime(time,units,CFcalendar);
