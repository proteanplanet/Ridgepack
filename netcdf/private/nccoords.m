function [nc]=nccoords(nc,name);

% NCCOORDS - Assigns coordinates attribute within a netcdf structure
%
% function [nc]=nccoords(nc,name);
%
...
if nargin<1 | nargin>2
 error('nccoords: Must have at one nc structure input and an option variable name')
elseif nargin>=1 & ~isstruct(nc); 
 error('nccoords: nc is not a structure')
end
...
[dimc,coor,dims,vars,mdim,mesh]=nccontent(nc);
...
for m = 1:numbervariables
  name=char(variablenames(m));
  coords=setdiff(ncvarcoords(nc,name),{'time'});
  if ~isempty(coords)
   nc.(name).coordinates=nccellcat(coords);
   removet=1;
  end
end
