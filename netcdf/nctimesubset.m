function [nc]=nctimesubset(ncfile,ncvar,months,yearstart,yearend)

% NCTIMESUBSET - Generate seasonal subsets of data based on consecutive months
%
% function [nc]=nctimesubset(ncfile,ncvar,months,yearstart,yearend)
%
...
% enter this command:
%
% nc=nctimesubset('timeseries.nc',{},[1 2 12],1990,1992);
%
...
%
% Written by Andrew Roberts 2015
...
if recreatemean
 ...
 if generate
  for i=index
   nc=ncclone(ncfile,ncvar,{'time'},{i}); % this specification sucks in time_bounds
   if i==index(1)
 ...
 nc=ncreduce(ncclone(fileout),{'time'},{},true);
 nc.attributes.comment=['Seasonal mean dataset only; ',...
                        ' Months: ',num2str(months,' %2.2i'),...
                        ', Years: ',num2str(ys),'-',num2str(ye)];
 ncwrite(nc,finalfile);
