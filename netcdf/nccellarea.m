function [nc]=nccellarea(nc,model)

% NCCELLAREA - Esimates the area grid cells for model or observational datasets
%
% function [nc]=nccellarea(nc,model)
%
...
% lat-long grid if need be (see ncsph2gen for more information). The output 
% is the same spherical or generalized coordinates that was input in nc 
% (see ncsph2gen for an explanation of each coordinate structure).
%
% Input:
% nc - netcdf structure (see ncstruct for more information) with 
%      geolocated data.
...
ncsquare=ncgridsquare(nc,model);
...
elseif model==3
  ...
  for i=1:dimi
  for j=1:dimj
   [X,Y,SLAT,K]=ncgeodetictoxy(nc.latitude.data(j,i),nc.longitude.data(j,i),SGN);
...
if model==1
 nc=ncadd(nc,'cell_area',cell_area,'area of grid cell on sphere',nc.longitude.dimension,'km^2');
elseif model==2
 nc=ncadd(nc,'cell_area',cell_area,'area of grid cell on wgs84 ellipsoid',nc.longitude.dimension,'km^2');
elseif model==3
 nc=ncadd(nc,'cell_area',cell_area,'area of grid cell on stereographic plane',nc.longitude.dimension,'km^2');
else
