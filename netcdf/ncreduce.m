function [ncr,nc]=ncreduce(nc,dimnames,bounds,varmean)

% NCREDUCE - Reduce dimensions in an nc structure by slicing or taking weighted means 
%
% function [ncr,nc]=ncreduce(nc,dimnames,bounds,varmean)
%
...
% Input:
%
% nc       - nc structure (see the help page of ncstruct for more information)
%
% dimnames - cell array of dimension variable names and order of dimensions to be 
%            removed from var. These may be specified in any order, with ncreduce 
...
%
% Implicit functionality of ncreduce:
%
...
%
% * ncreduce searches the nc structure for a variable called "mask" and uses this 
...
% nc=ncclone('era40.1958.nc','p2t'), and then is reduced using 
% [ncr,nc]=ncreduce(nc,{'longitude','time'},{[1 144],{'01-Jan-1958','03-Jan-1958'}}):
%
% >> nc=ncclone('era40.1958.nc','p2t')
%
...
[dimc,coor,dims,vars,mdim,mesh,supp]=nccontent(nc);

if isempty(ncunion(mdim,ncunion(dimc,dims))); ...
  error('nc contains no dimensions to be reduced'); ...
end
if isempty(ncunion(mesh,ncunion(coor,vars))); ...
  error('nc contains no variables to be reduced'); ...
end

% check that dimensions to be removed exist in the dataset
if isempty(intersect(dimnames,ncunion(mdim,ncunion(dimc,dims))));
 error(['No specified dimensions match variable dimensions: ',nccellcat(dimnames)])
end
...
if ~fasttrack
 ...
 if ~isempty(intersect(coor,dimnames)) 
	...
	nc=nccellarea(nc);
	nc=ncshuffle(nc,nc.cell_area.dimension);
 ...
 elseif ~isempty(setdiff(coor,{'latitude','longitude'})) 
	disp(['Don''t know how to calculate weightings for coordinates: ',...
             nccellcat(setdiff(coor,{'latitude','longitude'}))]);
	...
	disp(['Using pre-existing weightings for ',name]);
       ...
       if l>1
        ...
        if strcmpi(name,'time') & isfield(nc,'time_bounds')
         ...
         time_bounds=nctimeconvert(nc.time_bounds.data(:),...
        ...
        elseif strcmpi(name,'time') 
         ...
         % convert to netcdf time units so as to take calendar type into account
         time=nctimeconvert(nc.time.data(:),...
                            'days since 0000-01-01 00:00:0.0',...
                            nc.time.calendar,1);
...
if isfield(nc,'mask')
  ...
  nc=ncshuffle(nc,nc.mask.dimension);
...
for i=1:length(vars)
	...
	[dimremove,template,tempsize,endsize,ant,time]=...
                       ncvarbounds(nc,dimnames,bounds,name);
	...
	if length(dimremove)==length(nc.(name).dimension)
	 ...
	 if varmean | meanonly
	   ...
	   ncr.(name).long_name=[nc.(name).long_name,' ',nccellcat(ant),char(masked{i})];
         end
	 ncr.(name).data=squeeze(reshape(ncr.(name).data,endsize));
	 ncr.(name).dimension=ncsetdiff(nc.(name).dimension,dimremove);
...
if ~isempty(setdiff(coor,dimc));
 ...
 for i=1:length(coor)
  [dimremove,template,tempsize,endsize,ant]=ncvarbounds(nc,dimnames,bounds,char(coor{i}));
 ...
 if any(unchange)
	 ...
	 % remove them completely if required
	  ...
	  ncr.(lonvar).dimension=ncsetdiff(nc.(lonvar).dimension,dimremove);
	  ncr.(lonvar).long_name=['Mean longitude along track ',nccellcat(ant)];
	  ...
	  ncr.(latvar).dimension=ncsetdiff(nc.(latvar).dimension,dimremove);
	  ncr.(latvar).long_name=['Mean latitude along track ',nccellcat(ant)];
...
ncrdims=ncsetdiff(ncunion(dimc,dims),dimnames);
...
comment=['Reduced dataset via slices or means through:',nccellcat(dimnames)];
