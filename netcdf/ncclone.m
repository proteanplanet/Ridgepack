function [nc]=ncclone(ncfile,newvar,newrec,newrecp,newendp,nearestdate)

% NCCLONE - Clones data and its metadata from a netCDF file to a MATLAB structure
%
% function [nc]=ncclone(ncfile,newvar,newrec,newrecp,newendp,nearestdate)
%
% Input:
...
%          use of newrec. If you choose "year", then ncclone will take the 
      ...
      if nargin>2 && iscell(rec) ;
       ...
       if ~isempty(index) && ischar(recp{index})
        ...
        try
         ...
         timevec=nctimeconvert(double(input_data(:))*scale_factor+add_offset,...
                               units,calendar,0);
	...
	for ic=1:length(cdimids)
	 ...
	 if isempty(dimfind);
	  ...
	  disp(['coordinate variables for ',name,...
                ' must be explicitly listed in ncclone list.'])
          getcoords=0;
	 end
	end 
       else
      ...
      if isempty(newcoords)
       ...
       nc.(name).coordinates=nccellcat(newcoords);
...
for i=1:numbervariables
 ...
 if isfield(nc.(name),'units') & ...
  ...
  nc.(name).data=nctimeconvert(nc.(name).data(:),nc.(name).units,nc.(name).calendar,0);
  nc.(name).data=reshape(nc.(name).data,timeshape);

 % read WRF time data in character strings
 elseif strcmpi(name,'time') & isfield(nc.attributes,'TITLE') & ...
...
% ....................................................................
% remove time stamp character variables written in netCDF files by ncwrite (if it exists)
tosstchar=0;
[variablenames,numbervariables]=ncname(nc);
for i=1:numbervariables
 name=char(variablenames{i});
...
[dimc,coor,dims,vars]=nccontent(nc);
name=intersect(vars,newvar);
if length(name)==1
 nc=nccoords(nc,char(name{1}));
else
 nc=nccoords(nc);
end
