function [nc]=ncshuffle(nc,lastdims)

% NCSHUFFLE - Permute dimensions in an nc structure to a requested order
%
% function [nc]=ncshuffle(nc,lastdims)
%
...
%
% Input:
% nc       - input netcdf structure (see ncstruct for more information)
%
...
%
% nc=ncshuffle(nc,{'latitude','time','longitude'});
%
...
for m = 1:numbervariables
 ...
 elseif strcmp(nc.(var).dimension(end-length(name)+1:end),name)
  ...
  nc=ncpermute(nc,var,permutation);

  if debug; 
        disp([var,oldsize,'-> [',num2str(size(nc.(var).data)),...
        ']=[',nccellcat(nc.(var).dimension),' ]']);
  end
