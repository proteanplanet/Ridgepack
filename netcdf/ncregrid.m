function [ncnew]=ncregrid(nc,u,v,opmode,rec1,rec2,n,method)

% NCREGRID - Regrids data in nc structures to polar preset lat-long grids
%
% function [ncnew]=ncregrid(nc,u,v,opmode,rec1,rec2,n)
%
...
%             (see ncgridgen for further options)
%
...
%             using ncgridgen, or include the gridded fields x, y, 
%             latitude, longitude and a mask as is ouput from ncgridgen.
%
...
if length(nc.(u).dimension)>3 & ndims(squeeze(nc.(u).data))<=3
 try 
  nc=ncreduce(nc,setdiff(nc.(u).dimension,{'time'  'x'  'y'}));
 catch
...
[variablenames,numbervariables]=ncname(ncnew);
for m=1:numbervariables
	name=char(variablenames(m));
