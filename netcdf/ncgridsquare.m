function [nc,xdist,ydist]=ncgridsquare(nc,model)

% NCGRIDSQUARE - Generate vertices from generalized curvilinear grid cell central grid points
%
% function [nc,xdist,ydist]=ncgridsquare(nc,model)
%
...
%
% Input:
% nc    - netcdf structure (see ncstruct for more information).
%         This structure must be in generalized lat-lon coordinates
%         as described in the help page for ncsph2gen output. An example
...
if (isfield(nc,'x_corner') & isfield(nc,'x')) | ...
 ...
 nc=ncshuffle(nc,{'y_corner','x_corner'});
 nc=ncshuffle(nc,{'y','x'});
 return;
else
...
% Run quickly through ncsph2gen to check for correct setup
nc=ncsph2gen(nc);
...
elseif strcmp('regional',gridtype)
 ...
 if model==3
  ...
  for i=1:dimi+1
  for j=1:dimj+1
   [lat_corner(i,j),lon_corner(i,j),SLAT]=ncxytogeodetic(x_corner(i),y_corner(j),SGN);
  end
  end

  % apply grid rotation angle of original grid
  [lat,lon]=ncxytogeodetic(nc.x.data(1),nc.y.data(1),SGN);
...
nc=ncadd(nc,'x_corner',x_corner','x-coordinate vertex in Cartesian system',{'x_corner'},'','NC_FLOAT');
nc=ncadd(nc,'y_corner',y_corner','y-coordinate vertex in Cartesian system',{'y_corner'},'','NC_FLOAT');
...
nc=ncadd(nc,'latitude_corner',lat_corner,'latitude of corner grid points',{'x_corner','y_corner'},'degrees_north');
nc=ncadd(nc,'longitude_corner',lon_corner,'longitude of corner grid points',{'x_corner','y_corner'},'degrees_east');
